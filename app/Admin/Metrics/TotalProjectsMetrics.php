...
class TotalProjectsMetrics extends Card
{
    ...
    public function handle(Request $request)
    {
        ...
        $last_year = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subYear()->startOfYear(), Carbon::now()->subYear()->endOfYear()])->count();
        $last_years = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subYears(2)->startOfYear(), Carbon::now()->subYears(2)->endOfYear()])->count();
        $last_year_rate =  round($last_years == 0 ? 100 : ($last_year - $last_years) / $last_years, 2);


        $last_month = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subMonth()->startOfMonth(), Carbon::now()->subMonth()->endOfMonth()])->count();
        $last_months = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subMonths(2)->startOfMonth(), Carbon::now()->subMonths(2)->endOfMonth()])->count();
        $last_month_rate =  round($last_months == 0 ? 100 : ($last_month - $last_months) / $last_months, 2);

        $last_28_day = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subDays(28), Carbon::now()])->count();
        $last_28_days = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subDays(28 * 2), Carbon::now()->subDays(28)])->count();
        $last_28_days_rate =  round($last_28_days == 0 ? 100 : ($last_28_day - $last_28_days) / $last_28_days, 2);

        $last_7_day = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subDays(7), Carbon::now()])->count();
        $last_7_days = \App\Models\Project::whereBetween('created_at', [Carbon::now()->subDays(7*2), Carbon::now()->subDays(7)])->count();
